...
Mesh::Mesh(const size_t rows, const size_t cols)
    : rows(rows), cols(cols),
      currentGrid(std::make_unique<meshType>(rows, std::vector<Node>(cols))) {
...
const double Mesh::getNode(const size_t x, const size_t y) {
  const Node &currentNode = currentGrid->at(x).at(y);
...
void Mesh::setNode(const size_t x, const size_t y,
  ...
  Node &currentNode = currentGrid->at(x).at(y);
  ...
  for (auto i = 1u; i < rows - 1; i++) {
    for (auto j = 1u; j < cols - 1; j++) {
      std::swap(currentGrid->at(i).at(j).temperature,
                currentGrid->at(i).at(j).newTemperature);
    }
  }
}
